...
- `accounts` — **owned** automation identities (`user_id` = owner telegram id)
  ...
  - `phone_e164` is unique (no duplicate numbers) and is what the bot UI shows
  - `phone_mask` is a derived D1 column for non-UI/log contexts only
  - Rich fields: role, session secret name, workflow, status, last_login_at, …
- `login_sessions` / `user_states` — wizard state (ephemeral secrets; phone cleared after login)

### Where phone numbers live

| Location | What it stores |
|---|---|
| D1 `accounts.phone_e164` | Canonical full E.164 for the Forwarder / promo identity. Bot list/status/manage messages render this. |
| D1 `accounts.phone_mask` | Masked copy (e.g. `+98***11`) kept for non-UI storage. Not shown in Telegram. |
| D1 `login_sessions.phone` | Ephemeral full phone during the login wizard; cleared after login. |
| GitHub `TELEGRAM_SESSION_B64*` | Telethon session blob for that account. **Not** the phone number. |
| GitHub `LOGIN_PHONE` | Temporary manage.ps1 fallback only. Production login uses the Worker bridge. |
